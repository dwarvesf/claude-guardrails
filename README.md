...
3. **UserPromptSubmit secret scanner** — `scan-secrets.sh` (bash + jq) blocks prompts containing live credentials (AWS keys, GitHub / Anthropic / OpenAI tokens, PEM blocks, BIP39 phrases, Bitcoin WIF and BIP-32 xprv wallet keys). Prevents pasted secrets from reaching the model or landing in the on-disk session transcript.
